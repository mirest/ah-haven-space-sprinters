...
class TestUserModel(TestCase):
    def test_create_super_user(self):
        user = User.objects.create_superuser(username='admin',
                                             password='adminpassword', email='admin@sprinters.ug')
    ...
    def test_create_user_with_no_username(self):
        with self.assertRaises(TypeError):
            user = User.objects.create_user(
                username=None, email='sprinter1@space.com', password='mypassword')

    def test_return_str__method(self):
        self.user = User.objects.create_user(
            username="nandi", email="sprinter2@space.com", password='nandipassword')
        self.assertEqual(self.user.__str__(), 'sprinter2@space.com')

    def test_return_short_name__method(self):
        self.user = User.objects.create_user(
            username="leila", email="sprinter3@space.com", password='leilapassword')
