...
class BaseTest(APITestCase):
    ...
    def create_comment_with_highlighted_text_forward(self):
        self.client.credentials(HTTP_AUTHORIZATION=self.auth_header)
        self.comment = self.client.post(self.comment_url,
                                        data=json.dumps(
                                            {'comment_body': 'kawa', 'comment_on_start': '0', 'comment_on_end': '5'}),
                                        content_type='application/json')

    def create_comment_with_highlighted_text_backward(self):
        self.client.credentials(HTTP_AUTHORIZATION=self.auth_header)
        self.comment = self.client.post(self.comment_url,
                                        data=json.dumps(
                                            {'comment_body': 'kawa', 'comment_on_start': '5', 'comment_on_end': '0'}),
                                        content_type='application/json')

    def create_comment_with_highlighted_text_not_in_article_forward(self):
        self.client.credentials(HTTP_AUTHORIZATION=self.auth_header)
        self.comment = self.client.post(self.comment_url,
                                        data=json.dumps(
                                            {'comment_body': 'kawa', 'comment_on_start': '20', 'comment_on_end': '0'}),
                                        content_type='application/json')

    def create_comment_with_highlighted_text_not_in_article_backward(self):
        self.client.credentials(HTTP_AUTHORIZATION=self.auth_header)
        self.comment = self.client.post(self.comment_url,
                                        data=json.dumps(
                                            {'comment_body': 'kawa', 'comment_on_start': '0', 'comment_on_end': '20'}),
                                        content_type='application/json')
